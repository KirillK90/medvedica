...
function dt_paginator( $query = null, $opts = array() ) {
	...
	if ( !is_single() ) {
		$defaults = array(
			...
			'item_wrap'         => '<a href="%HREF%" %CLASS_ACT% data-page-num="%PAGE_NUM%">%TEXT%</a>',
			'first_wrap'        => '<a href="%HREF%" %CLASS_ACT% data-page-num="%PAGE_NUM%">%FIRST_PAGE%</a>',
			'last_wrap'         => '<a href="%HREF%" %CLASS_ACT% data-page-num="%PAGE_NUM%">%LAST_PAGE%</a>',
			'pages_wrap'        => '',
			'next_page_wrap'    => '%LINK%',
			'prev_page_wrap'    => '%LINK%',
			'ajaxing'           => false,
