...
class The7_Registration_Warning {
	...
	public static function add_admin_notices() {
		...
		if ( $domains_count > 4 ) {
			the7_admin_notices()->add( 'the7_registration_hard_warning', array( __CLASS__, 'registration_hard_warning' ), 'the7-dashboard-notice notice-error' );
		} elseif ( $domains_count > 2 ) {
	...
	public static function registration_hard_warning() {
		$domains_count = self::get_domains_count();

		include dirname( __FILE__ ) . '/views/html-notice-hard-warning.php';
	}
