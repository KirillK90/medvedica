...
if(!class_exists("Ultimate_Team")){
	class Ultimate_Team{
		...
		function ult_team_shortcode($atts, $content){
			...
			$title_box_padding = $title_box_margin = $team_css = $team_member_responsive_enable = $team_responsive_width = $target = $link_title  = $rel = '';
			...
			if ( $team_member_style == 'style-3' ) {
				...
				if ( $link_switch == 'on' && $staff_link != '' ) {
			        ...
			        echo '<a '. Ultimate_VC_Addons::uavc_link_init($url, $target, $title, $rel ).'>';
			    }
			    ...
			    if ( $link_switch == 'on' && $staff_link != '' ) {
			        ...
			        echo '<a '. Ultimate_VC_Addons::uavc_link_init($url, $target, $title, $rel ).' style="text-decoration: none;" >';
			    }
